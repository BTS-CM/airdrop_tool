...
function humanReadableFloat(satoshis, precision) {
    return parseFloat((satoshis / Math.pow(10, precision)).toFixed(precision));
}
...
async function getAsset(node, searchInput) {
    try {
      ...
      let filteredSymbols = symbols.filter((x) => x !== null);
...
export default function PerformAirdrop(properties) {
    ...
    let sortedWinners = winners.sort((a,b) => b.qty - a.qty);
    let tokenRows = [];
    ...
    for (let i = 0; i < sortedWinners.length; i++) {
        ...
        if (distroMethod === "Proportionally") {
            ...
            let currentWinner = sortedWinners[i];
            ...
            let filteredRows = tokenRows.filter(x => x.id !== currentWinner.id)
                    ...
                    ?   valid
                        .map((winner) => {   
                            return <tr key={winner.id}>
                                        ...
                                        <td>
                                            { winner.assignedTokens.toFixed(tokenDetails.precision) } {tokenName ? tokenName : assetName}
                                        </td>
                                    </tr>
                        })
                    :   null;
        ...
        invalidRows = tokenRows
                        ...
                        .map((winner) => {   
                            return <tr key={winner.id}>
                                        ...
                                        <td>
                                            0 {tokenName ? tokenName : assetName}
                                        </td>
                                    </tr>
                        })
    }

    let airdropCards = winnerChunks && winnerChunks.length
                        ...
                        <Card shadow="md" radius="md" padding="xl" mt={20}>
                            ...
                            }
                            {
                                validRows && validRows.length
                                    ?   <>
                                            <Text>
                                                <HiOutlineEmojiHappy /> Ticket holders included in airdrop
                                            </Text>
...

                            {
                                invalidRows && invalidRows.length
                                    ? <>
                                        <Text mt="md">
                                            <HiOutlineEmojiSad /> Disqualified from airdrop
                                        </Text>
                                        <Text c="dimmed" mb="sm">
                                    ...
                                    !validRows || !validRows.length
                                        ?   <Card shadow="md" radius="md" padding="sm" style={{backgroundColor:'#FAFAFA'}}>
                                                <Text fz="lg" fw={500} mt="md">
                                                    <HiOutlineShieldExclamation /> Nothing to airdrop
                                                </Text>
                                                <Text fz="sm" c="dimmed" mt="xs">
                                        ...
                                        :   <Card shadow="md" radius="md" padding="sm" style={{backgroundColor:'#FAFAFA'}}>
                                                <Text fz="lg" fw={500} mt="md">
                                                    <HiOutlineShieldCheck /> Proceed with airdrop?
                                                </Text>
                                                <Text fz="sm" c="dimmed" mt="xs">
